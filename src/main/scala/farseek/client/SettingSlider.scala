...
class SettingSlider(setting: NumericSetting, index: Int, _xPosition: Int, _yPosition: Int, _width: Int, _height: Int)
    ...
    private def setValueFromPosition(x: Int) {
        setValue(setting.snapToStep(setting.denormalized(clamp((x - (x + 4)).toDouble / (width - 8).toDouble, 0F, 1F))))
    }
    ...
    override protected def mouseDragged(game: Minecraft, x: Int, y: Int) {
        ...
        drawTexturedModalRect(x + (normalizedValue * (width - 8).toDouble).toInt    , y,   0, 66, 4, 20)
        drawTexturedModalRect(x + (normalizedValue * (width - 8).toDouble).toInt + 4, y, 196, 66, 4, 20)
    }
