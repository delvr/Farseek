...
/** A [[ClassPatcher]] that replaces any method matching the provided [[MethodReplacement]] with an implementation
  * that calls a static method specified by the replacement. This method must have the same parameters as the replaced method, plus
  * a [[ReplacedMethod]] argument for calling the original method implementation (created here as a new method, with an aliased name).
  * If replacing a non-static method, the replacement method must also have an additional parameter for the receiving instance.
  *
  * The original method implementation will have one prefix in its name. Each successive replacement adds a new prefix in the chain. For example:
  *
  * `method` (2nd replacement) `->` `REPLACED_REPLACED_method` (1st replacement) `->` `REPLACED_method` (original)
  *
  * Methods are matched by name/descriptor only, without knowledge of whether the visited class is a subclass of the one specified
  * by `replacement`. This is to avoid loading other classes in search of a superclass, which often causes problems in FML.
  * (Stack frames are manually visited instead of generated by COMPUTE_FRAMES in the class writer, for the same reason.)
  * The replacement method includes a runtime check for compatibilty with the replacement-specified class, and directly calls the original
  * implementation if not the same or a subclass of that class.
  *
  * @author delvr
  */
class MethodReplacer(bytecode: Array[Byte], className: String, replacement: MethodReplacement) extends ClassPatcher(bytecode) {

    import farseek.core.MethodReplacement._
    import farseek.core.ReplacedMethodFactory._

    private val methodReplacements = mutable.Map[String, mutable.Set[MethodReplacement]]().withDefaultValue(mutable.Set.empty)
    ...
    override def visitMethod(accessFlags: Int, name: String, descriptor: String, signature: String, exceptions: Array[String]) = {
        ...
        // Don't visit abstract methods, non-matching methods, or static methods of a different class (they can't be overrides of our target).
    ...
    private def replaceMethod(aliasedName: String, visitor: GeneratorAdapter, replacement: MethodReplacement, isStatic: Boolean) {
        ...
        val returnType = if(replacement.descriptor.endsWith(")V")) Array[AnyRef]() else Array(returnFrameType(replacement.descriptor))
        val endLabel = new Label
        visitor.visitCode()
        if(className == replacement.className)
            debug(s"Patching method ${replacement.devName}/${replacement.srgName} on class $className")
        else { // Create a runtime check to see if this is this is a subclass of replacement.className
            trace(s"Patching method ${replacement.devName}/${replacement.srgName} on possible subclass of ${replacement.className}: $className")
            ...
            visitor.loadThis()
            visitor.loadArgs()
            visitor.visitMethodInsn(INVOKESPECIAL, className, aliasedName, replacement.descriptor, false) // Call the original implementation
        ...
        visitor.visitMethodInsn(INVOKESTATIC, replacement.replacementClass, replacement.replacementMethod,
            replacement.descriptor.replace(")", ReplacementMethodType + (if(isStatic) "" else typeDescriptor(replacement.className)) + ')'), false)
