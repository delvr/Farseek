...
/** Farseek implementation of [[IClassTransformer]].
  * Validates that no net.minecraft classes are loaded during transformation.
        ...
        if(bytecode eq null) null else {
            ...
            if(excludedClassPrefixes.exists(name.startsWith)) {
                ...
                transforming.foreach { current =>
                    require(!isMinecraftClass(name),
                        s"Minecraft class $name loaded while transforming class $current by core mod $this")
                }
                transforming = Some(name)
                try transform(name, bytecode)
                finally { transforming = None } // Need to be set for when Forge recovers from non-fatal errors
                                                // like loading a non-existent Optifine "Config" class
            }
        }
    }
