...
object SaveHandlerExtensions {
    ...
    /** Method patch for SaveHandler.readData that allows Farseek mods to warn about, or forbid, loading existing worlds created without them. */
    def readData(handler: SaveHandler, worldInfo: WorldInfo, properties: java.util.Map[String, NBTBase], tags: NBTTagCompound,
                 super_readData: ReplacedMethod[FMLContainer])(implicit container: FMLContainer) {
        ...
        val missingFarseekMods: Seq[FarseekBaseMod] = loadedModContainers.map(_.getMod).collect { case fm: FarseekBaseMod if !savedModIds.contains(fm.id) => fm }
